...
- The installer and helper scripts detect `mise` at its actual install location and fall back to plain `uv` for interactive use; the systemd service requires `mise` + `uv` for reliable operation
